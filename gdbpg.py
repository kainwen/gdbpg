...
def format_plan_tree(tree, indent=0):
    ...
    if is_a(tree, 'Hash'):
        hash = cast(tree, 'Hash')
        node_extra += '   <rescannable=%(rescannable)s>\n' %{
...
def format_node(node, indent=0):
    ...
    if is_a(node, 'TargetEntry'):
        ...
        retval = 'TargetEntry (resno=%(resno)s resname=%(name)s origtbl=%(tbl)s origcol=%(col)s junk=%(junk)s])' % {
            'resno': node['resno'],
            'name': name,
            'tbl': node['resorigtbl'],
            'col': node['resorigcol'],
...
def format_rte(node, indent=0):
    retval = 'RangeTblEntry (rtekind=%(rtekind)s relid=%(relid)s)' % {
        'relid': node['relid'],
        'rtekind': node['rtekind'],
    }
...
def format_relabel_type(node, indent=0):
    ...
    retval = """RelabelType [resulttype=%(resulttype)s resulttypmod=%(resulttypmod)s relabelformat=%(relabelformat)s location=%(location)s]\n""" % {
